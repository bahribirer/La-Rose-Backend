...
@router.post("/admin/seed-web-fields", dependencies=[Depends(admin_required)])
async def seed_web_fields():
    """
    47 ürüne slug ve name_tr alanlarını tek seferlik yazar.
    Admin panelden çalıştırılır.
    """
    from pymongo import UpdateOne
    ...
    operations = [
        UpdateOne({"gtin": gtin}, {"$set": data})
        for gtin, data in WEB_PRODUCTS.items()
    ]
    ...
    return {
        "message": f"{result.modified_count} ürün güncellendi",
