...
const masterIds = masterList.map(item => item.id);

// 1. Delete products not in the master list
const deleteResult = db.products.deleteMany({ id: { $nin: masterIds } });
console.log(`Deleted ${deleteResult.deletedCount} extra products.`);

// 2. Update all 47 products, unset gtin field, and normalize data
let updatedCount = 0;
...
console.log(`Normalized ${updatedCount} products. DB now contains exactly 47 master products.`);
